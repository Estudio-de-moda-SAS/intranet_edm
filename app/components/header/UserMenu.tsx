 ...
 *
 * @remarks
 * **Cambios respecto a la version NextAuth:**
 * - El boton de cerrar sesion ya no apunta a `/api/auth/signout`.
 *   Ahora llama a {@link logout} de MSAL que ejecuta `logoutRedirect`
 *   y redirige al login corporativo.
 * - Las notificaciones muestran el panel vacio con un aviso honesto
 *   mientras no se agreguen los scopes `Mail.Read` y `Calendars.Read`.
 */
...
type MenuAction = 'navigate' | 'panel' | 'modal' | 'action';
...
export default function UserMenu({ user }: Props) {
  ...
  /**
   * Cierra la sesion del colaborador usando MSAL logoutRedirect.
   * Muestra estado de carga mientras se procesa para evitar doble clic.
   */
  const handleLogout = async () => {
    ...
    try {
      await logout();
      // logout() hace redirect — el codigo siguiente no se ejecuta
    } catch (err) {
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          {/* Sign out — ahora usa logout() de MSAL */}
          <div className="px-2 py-2 border-t border-slate-100 dark:border-[#21262d]">
            <button
              onClick={handleLogout}
              disabled={loggingOut}
              className="w-full flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] transition-colors group
                         text-rose-500 hover:bg-rose-50 disabled:opacity-50 disabled:cursor-not-allowed
