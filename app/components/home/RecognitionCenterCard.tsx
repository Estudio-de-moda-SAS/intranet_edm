/**
 * @module RecognitionCenterCard
 * Componente cliente para mostrar los reconocimientos recibidos por el usuario.
 *
 * @remarks
 * Renderiza una tarjeta con listado de reconocimientos, categoría visual,
 * expansión por elemento y estado vacío cuando no hay datos.
 */
...
/**
 * Configuración visual por categoría de reconocimiento.
 */
...
/**
 * Props del componente {@link RecognitionsCard}.
 */
interface Props {
  /**
   * Lista de reconocimientos del usuario.
   */
  recognitions: Recognition[];
}

/**
 * Renderiza la tarjeta de reconocimientos.
 *
 * @param props Propiedades del componente.
 * @param props.recognitions Lista de reconocimientos.
 * @returns Tarjeta con listado expandible o estado vacío.
 *
 * @remarks
 * - Permite expandir un reconocimiento para ver su mensaje completo.
 * - Muestra avatar o foto del emisor.
 * - Usa estilos por categoría mediante `CATEGORY_CONFIG`.
 */
export function RecognitionsCard({ recognitions }: Props) {
  /**
   * ID del reconocimiento actualmente expandido.
   */
    ...
    >
      ...
      <div className="flex items-center justify-between px-[18px] py-4 border-b
        ...
        <div className="flex items-center gap-[9px]">
          <span
            className="flex h-[34px] w-[34px] shrink-0 items-center justify-center rounded-[10px] text-[16px]"
            style={{ background: "linear-gradient(135deg, #f59e0b 0%, #d97706 100%)" }}
          >
            🏅
          </span>
          <div>
            <h2 className="text-[14px] font-bold leading-[1.2] text-slate-900 dark:text-[#e6edf3]">
              Mis Reconocimientos
            </h2>
            <p className="text-[11px] mt-[1px] text-slate-400 dark:text-[#545d68]">
        ...
        <span
          className="flex h-7 min-w-[28px] items-center justify-center rounded-full text-[12px] font-extrabold
              ...
              >
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-[12.5px] font-bold truncate text-slate-800 dark:text-[#e6edf3]">
                      {rec.title}
                    </p>
                    <p className="text-[11px] mt-[1px] text-slate-400 dark:text-[#545d68]">
                      De{" "}
                      <strong className="font-semibold text-slate-600 dark:text-[#adbac7]">
                        {rec.from}
                      </strong>
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span
                      className={`flex items-center gap-[3px] rounded-full px-[7px] py-[2px] text-[10px] font-bold
                ...
                {isOpen && (
                  <div className="px-3 pb-3 border-t border-slate-200/60 dark:border-[#30363d]">
                    <p className="mt-2.5 mb-1.5 text-[12px] leading-relaxed italic text-slate-600 dark:text-[#adbac7]">
      ...
      <div className="px-3 pt-2.5 pb-3">
        <button
          className="w-full rounded-lg border border-dashed py-2 text-[12px] font-semibold transition-colors
