...
const STATUS_CONFIG: Record<TaskStatus, { dot: string; bg: string; text: string; label: string }> = {
  pendiente:   { dot: "bg-slate-400",   bg: "bg-slate-50 border-slate-200",     text: "text-slate-600",   label: "Pendiente"   },
  en_progreso: { dot: "bg-amber-400",   bg: "bg-amber-50 border-amber-200",     text: "text-amber-700",   label: "En progreso" },
  completada:  { dot: "bg-emerald-400", bg: "bg-emerald-50 border-emerald-200", text: "text-emerald-700", label: "Completada"  },
  bloqueada:   { dot: "bg-rose-500",    bg: "bg-rose-50 border-rose-200",       text: "text-rose-700",    label: "Bloqueada"   },
};

// ✅ Fix: constantes con fallback para evitar acceso undefined
...
function Avatar({ initials, color = "bg-violet-500" }: { initials: string; color?: string }) {
  return (
    <span className={cn(
      ...
      "text-[10px] font-semibold text-white ring-1 ring-white",
      color
    )}>
      {initials}
    </span>
...
function TaskDetailPanel({ task }: { task: TaskDetail }) {
  // ✅ Fix: ?? DEFAULT_PRIORITY / DEFAULT_STATUS garantizan que nunca sean undefined
      ...
      <div className="mx-4 mb-3 rounded-xl border border-slate-100 bg-slate-50/70 overflow-hidden">
        ...
        <div className="p-4 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {isFromPlanner ? (
              <span className="inline-flex items-center gap-1.5 rounded-full border border-[#e0dff5] bg-[#f2f1fb] px-2.5 py-0.5 text-[11px] font-medium text-[#5b5ea6]">
                <PlannerIcon className="h-3 w-3 text-[#5b5ea6]" />
                Microsoft Planner
              </span>
            ) : task.sourceSystem ? (
              <span className="inline-flex items-center gap-1.5 rounded-full border border-violet-100 bg-violet-50 px-2.5 py-0.5 text-[11px] font-medium text-violet-600">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className={cn(
                ...
                task.priority === "alta" ? "text-rose-600" : "text-slate-700"
              )}>
                <Clock className={cn("h-3.5 w-3.5", task.priority === "alta" ? "text-rose-400" : "text-slate-400")} />
                {fmtFecha(task.due)}
              </span>
            </div>
          ...
          {/* Descripción */}
          {task.description && (
            <div className={cn("text-[12px] leading-relaxed text-slate-500", p.detailAccent)}>
              {task.description}
            </div>
          )}
          ...
          {task.tags?.length ? (
            <div className="flex flex-wrap gap-1.5">
              {task.tags.map((tag) => (
                <span
                  key={tag}
                  className="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-white px-2 py-0.5 text-[11px] text-slate-500"
                >
          ...
          <div className="flex items-center gap-2 pt-0.5">
            ...
            <button className={cn(
              ...
              "text-[11.5px] font-medium text-slate-600 transition-colors hover:bg-slate-50 hover:border-slate-300"
            )}>
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
        ...
        {!isEmpty && (
          <span className="rounded-full bg-violet-50 border border-violet-100 px-2.5 py-0.5 text-[11px] font-semibold text-violet-600 tabular-nums">
            {tasks.length}
          </span>
        )}
      </div>
        ...
        {isEmpty ? (
          <div className="flex flex-col items-center gap-3 py-12 text-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-slate-50 border border-slate-100">
              <PartyPopper className="h-5 w-5 text-slate-300" />
            </div>
            <div>
              <p className="text-[13px] font-semibold text-slate-500">Todo al día</p>
              <p className="text-[11px] text-slate-300 mt-0.5">No tienes tareas pendientes en Planner</p>
            </div>
          </div>
        ) : (
              ...
              return (
                <li key={task.id}>
                  ...
                  >
                    ...
                    <span className={cn("h-2 w-2 shrink-0 rounded-full ring-2 ring-white", p.dot)} />
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-2.5 flex-wrap">
                        ...
                        <span className={cn(
                          ...
                          isUrgent ? "text-rose-500 font-medium" : "text-slate-400"
                        )}>
                        ...
                        {task.plannerUrl ? (
                          <span className="flex items-center gap-1 text-[10.5px] font-medium text-[#5b5ea6]">
                            <PlannerIcon className="h-3 w-3" />
                            Planner
                          </span>
                        ) : task.sourceSystem ? (
                          <span className="text-[10.5px] font-medium text-violet-400">
                            {task.sourceSystem}
                          </span>
                        ) : null}
                      </div>
      ...
      {!isEmpty && (
        <div className="flex items-center justify-between px-5 py-2.5 border-t border-slate-100 bg-slate-50/50 shrink-0">
          ...
          <button className="inline-flex items-center gap-1 text-[11px] font-medium text-violet-500 hover:text-violet-700 transition-colors">
