/**
 * @module BirthdayCard
 * Componentes para mostrar la tarjeta de cumpleaños del home.
 *
 * @remarks
 * Este archivo renderiza un listado de cumpleaños del día, incluyendo
 * avatar por iniciales, fecha formateada y acceso rápido al perfil.
 */
...
/**
 * Genera un color base para el avatar según el nombre.
 *
 * @param name Nombre de la persona.
 * @returns Color HSL usado como fondo del avatar.
 */
...
/**
 * Props del componente {@link Initials}.
 */
interface InitialsProps {
  /**
   * Nombre usado para calcular las iniciales.
   */
  name: string;
}

/**
 * Renderiza un avatar circular con iniciales.
 *
 * @param props Propiedades del componente.
 * @param props.name Nombre completo de la persona.
 * @returns Avatar textual con color dinámico.
 */
function Initials({ name }: InitialsProps) {
...
/**
 * Props del componente {@link BirthdaysCard}.
 */
interface Props {
  /**
   * Lista de cumpleaños a mostrar.
   */
  birthdays: Birthday[];
}

/**
 * Renderiza la tarjeta de cumpleaños.
 *
 * @param props Propiedades del componente.
 * @param props.birthdays Lista de cumpleaños del día.
 * @returns Tarjeta con listado de cumpleaños o estado vacío.
 *
 * @remarks
 * - Si no hay elementos, muestra un mensaje vacío.
 * - Si existen cumpleaños, muestra nombre, fecha, área y acceso al perfil.
 */
export function BirthdaysCard({ birthdays }: Props) {
          ...
          birthdays.map((b) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-slate-800 dark:text-[#e6edf3]">
                  {b.name}
                </p>
                <p className="text-[11px] text-slate-400 dark:text-[#545d68]">
                  ...
                  {b.area ? (
                    <>
                      {" "}
                      <span className="text-slate-300 dark:text-[#30363d]">·</span> {b.area}
                    </>
                  ) : null}
                </p>
              </div>
