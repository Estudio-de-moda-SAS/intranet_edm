...
const CATEGORIES: { key: FeedbackCategory; label: string; icon: typeof Lightbulb; color: string; bg: string }[] = [
  { key: "experiencia", label: "Experiencia general", icon: HeartHandshake, color: "text-violet-600 dark:text-violet-400", bg: "bg-violet-50 group-hover:bg-violet-100 border-violet-100 group-hover:border-violet-200 dark:bg-violet-500/[0.08] dark:group-hover:bg-violet-500/[0.15] dark:border-violet-500/20 dark:group-hover:border-violet-500/30" },
  { key: "sugerencia",  label: "Sugerencia",          icon: Lightbulb,      color: "text-amber-600 dark:text-amber-400",   bg: "bg-amber-50 group-hover:bg-amber-100 border-amber-100 group-hover:border-amber-200 dark:bg-amber-500/[0.08] dark:group-hover:bg-amber-500/[0.15] dark:border-amber-500/20 dark:group-hover:border-amber-500/30"     },
  { key: "problema",    label: "Reportar problema",   icon: Bug,            color: "text-rose-500 dark:text-rose-400",     bg: "bg-rose-50 group-hover:bg-rose-100 border-rose-100 group-hover:border-rose-200 dark:bg-rose-500/[0.08] dark:group-hover:bg-rose-500/[0.15] dark:border-rose-500/20 dark:group-hover:border-rose-500/30"         },
];
...
export function FeedbackPanel() {
  ...
  function openWith(cat: FeedbackCategory) { setCategory(cat); setOpen(true); }

  function handleSubmit() {
    if (!rating || !message.trim()) return;
    setSent(true);
    setTimeout(() => { setOpen(false); setSent(false); setRating(null); setHover(null); setMessage(""); setCategory(null); }, 2200);
  }
  ...
  return (
    <>
      <section className="rounded-xl border shadow-sm overflow-hidden
        ...
        <div className="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-800 dark:text-[#e6edf3]">Tu opinión importa</h3>
      ...
      <Modal
        ...
        title={category ? CATEGORIES.find((c) => c.key === category)?.label ?? "Enviar Feedback" : "Enviar Feedback"}
                ...
                {([1, 2, 3, 4, 5] as Rating[]).map((n) => (
                  ...
                  >
                    <Star className={`h-7 w-7 transition-colors ${
                      activeRating && n <= activeRating ? "fill-amber-400 text-amber-400" : "text-slate-200 dark:text-[#30363d]"
                    }`} />
                  </button>
                ))}
              </div>
