...
export default function GlobalSearchResults({ results, query, onSelect }: Props) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') { e.preventDefault(); setActiveIndex((p) => (p + 1) % flatResults.length); }
      if (e.key === 'ArrowUp')   { e.preventDefault(); setActiveIndex((p) => (p - 1 + flatResults.length) % flatResults.length); }
      if (e.key === 'Enter') {
        e.preventDefault();
        const selected = flatResults[activeIndex];
        if (selected) { router.push(selected.href); onSelect?.(); }
      }
    };
  ...
  const highlightText = (text: string, query: string) => {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === query.toLowerCase()
        ? <span key={i} className="font-semibold text-violet-700 dark:text-violet-400">{part}</span>
        : part
    );
  };
                ...
                {items.map((app: any) => {
                  ...
                  return (
                    <motion.div
                      ...
                      onClick={() => { router.push(app.href); onSelect?.(); }}
