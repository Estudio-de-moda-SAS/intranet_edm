...
import {
  Users, TrendingDown, UserCheck, Activity,
  BarChart2, Target,
} from "lucide-react";
...
export default function HRDashboard() {
  return (
    <PageEnter className="space-y-10">
      ...
      {/* ── 1. Headcount trend ─────────────────────────────────── */}
      <AnimateItem>
        <SectionHeading icon={Users} label="Evolución de Headcount" sub="Empleados activos, ingresos y salidas — últimos 6 meses" />
          ...
          <div className="h-60">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={HEADCOUNT_TREND}>
                ...
                <Area  type="monotone" dataKey="activos" name="Activos"  stroke="#8b5cf6" fill="url(#actGrad)" strokeWidth={2.5} dot={false} activeDot={{ r: 4, strokeWidth: 0 }} />
                <Bar   dataKey="nuevos"  name="Ingresos" fill="#10b981"  radius={[3,3,0,0]} />
                <Bar   dataKey="salidas" name="Salidas"  fill="#fca5a5"  radius={[3,3,0,0]} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <AnimateItem>
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          ...
          <div className="lg:col-span-3">
            <SectionHeading icon={TrendingDown} label="Rotación vs. Industria" sub="% mensual" />
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="h-52">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={TURNOVER}>
                    ...
                    <Line type="monotone" dataKey="rotacion"   name="EDM"       stroke="#8b5cf6" strokeWidth={2.5} dot={false} activeDot={{ r: 4, strokeWidth: 0 }} />
                    <Line type="monotone" dataKey="industria"  name="Industria" stroke="#cbd5e1" strokeWidth={2} strokeDasharray="4 4" dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="lg:col-span-2">
            <SectionHeading icon={Target} label="Engagement" sub="Encuesta trimestral" />
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="h-52">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart data={ENGAGEMENT_RADAR}>
                    <PolarGrid stroke="#f1f5f9" />
                    <PolarAngleAxis dataKey="area" tick={{ fontSize: 10, fill: "#94a3b8" }} />
                    <Radar name="Engagement" dataKey="score" stroke="#8b5cf6" fill="#8b5cf6" fillOpacity={0.15} strokeWidth={2} />
      ...
      <AnimateItem>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div>
            <SectionHeading icon={BarChart2} label="Desempeño por Departamento" />
                ...
                <div key={dept.dept}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-3 text-[11px]">
                      <span className="text-slate-400">Satisfacción: <span className="font-semibold text-slate-600">{dept.satisfaccion}%</span></span>
                      <span className="font-bold text-violet-600">{dept.desempeno}%</span>
                    </div>
                  </div>
              ...
              <ul className="divide-y divide-slate-50">
                {RECENT_ACTIVITY.map((item, i) => (
                  <li key={i} className="flex items-center gap-3 px-5 py-3 hover:bg-slate-50/50 transition-colors">
                    <span className={`h-2 w-2 shrink-0 rounded-full ${EVENT_DOT[item.type]}`} />
      ...
      {/* ── 4. Dept detail table ───────────────────────────────── */}
      <AnimateItem>
        <SectionHeading icon={UserCheck} label="Resumen de Personal por Departamento" />
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {["Departamento", "Activos", "Desempeño", "Satisfacción", "Ausentismo", "Estado"].map((col) => (
            ...
            <tbody className="divide-y divide-slate-50">
              {DEPT_PERFORMANCE.map((dept, i) => {
                const isHealthy = dept.desempeno >= 85 && dept.satisfaccion >= 80;
                const isWarning = dept.desempeno < 80 || dept.ausentismo > 1.8;
                return (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-3 text-[13px] font-semibold text-slate-800">{dept.dept}</td>
                    <td className="px-5 py-3 text-[13px] tabular-nums text-slate-600">—</td>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <div className="h-1.5 w-16 rounded-full bg-slate-100 overflow-hidden">
                          <div className="h-full rounded-full bg-violet-500" style={{ width: `${dept.desempeno}%` }} />
                        </div>
                        <span className="text-[12px] font-semibold text-slate-700 tabular-nums">{dept.desempeno}%</span>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-[13px] font-medium tabular-nums text-slate-600">{dept.satisfaccion}%</td>
                    <td className="px-5 py-3">
                      <span className={`text-[13px] font-semibold tabular-nums ${dept.ausentismo > 1.8 ? "text-rose-600" : "text-emerald-600"}`}>
                        {dept.ausentismo}%
                      </span>
                    </td>
                    <td className="px-5 py-3">
                      <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${
                        isHealthy  ? "bg-emerald-50 border-emerald-100 text-emerald-700" :
                        isWarning  ? "bg-amber-50 border-amber-100 text-amber-700" :
                                     "bg-violet-50 border-violet-100 text-violet-700"
                      }`}>
                        {isHealthy ? "Saludable" : isWarning ? "Atención" : "Normal"}
                      </span>
                    </td>
                  </tr>
                );
