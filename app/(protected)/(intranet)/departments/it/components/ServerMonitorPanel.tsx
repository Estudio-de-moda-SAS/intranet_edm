// app/it/components/ServerMonitorPanel.tsx
"use client";
...
function genHistory(baseCpu: number, baseRam: number): ServerMetric[] {
  ...
  return hours.map((h, i) => ({
    t:   `${h}:00`,
    cpu: Math.min(99, Math.max(1, baseCpu + Math.round(Math.sin(i * 0.9) * 12 + (Math.random() - 0.5) * 8))),
    ram: Math.min(99, Math.max(10, baseRam + Math.round(Math.cos(i * 0.7) * 6 + (Math.random() - 0.5) * 4))),
...
const SERVERS: Server[] = [
  {
    id: "sv1", name: "SRV-APP-01",  role: "Aplicaciones",   os: "Windows Server 2022",
    ip: "10.0.1.10", location: "Rack A · Piso 3",
    status: "online",      cpu: 42, ram: 67, disk: 54, temp: 48,
    uptime: "32 días", lastPatch: "2026-02-28",
    history: genHistory(42, 67),
  },
  {
    id: "sv2", name: "SRV-DB-01",   role: "Base de datos",  os: "Windows Server 2022",
    ip: "10.0.1.11", location: "Rack A · Piso 3",
    status: "warning",     cpu: 78, ram: 82, disk: 71, temp: 61,
    uptime: "32 días", lastPatch: "2026-02-28",
    history: genHistory(78, 82),
  },
  {
    id: "sv3", name: "SRV-FILE-01", role: "Archivos / NAS",  os: "Ubuntu Server 22.04",
    ip: "10.0.1.12", location: "Rack B · Piso 3",
    status: "online",      cpu: 23, ram: 45, disk: 88, temp: 41,
    uptime: "18 días", lastPatch: "2026-03-01",
    history: genHistory(23, 45),
  },
  {
    id: "sv4", name: "SRV-BKUP-01", role: "Backups / DR",    os: "Ubuntu Server 22.04",
    ip: "10.0.1.13", location: "Rack B · Piso 3",
    status: "maintenance", cpu: 5,  ram: 14, disk: 62, temp: 35,
    uptime: "—", lastPatch: "2026-03-10",
    history: genHistory(5, 14),
  },
  {
    id: "sv5", name: "SRV-WEB-01",  role: "Web / Intranet",  os: "Ubuntu Server 22.04",
    ip: "10.0.1.14", location: "Rack C · Piso 2",
    status: "online",      cpu: 31, ram: 52, disk: 43, temp: 44,
    uptime: "11 días", lastPatch: "2026-03-05",
    history: genHistory(31, 52),
  },
  {
    id: "sv6", name: "SRV-SEC-01",  role: "Seguridad / SIEM", os: "CentOS Stream 9",
    ip: "10.0.1.15", location: "Rack A · Piso 2",
    status: "online",      cpu: 55, ram: 71, disk: 39, temp: 52,
    uptime: "25 días", lastPatch: "2026-02-20",
...
const STATUS: Record<ServerStatus, { label: string; dot: string; badge: string; icon: React.ReactNode }> = {
  online:      { label: "Online",       dot: "bg-emerald-400",              badge: "bg-emerald-50 text-emerald-700 border-emerald-200", icon: <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> },
  warning:     { label: "Advertencia",  dot: "bg-amber-400 animate-pulse",  badge: "bg-amber-50 text-amber-700 border-amber-200",       icon: <AlertTriangle className="w-3.5 h-3.5 text-amber-500" /> },
  critical:    { label: "Crítico",      dot: "bg-red-500 animate-pulse",    badge: "bg-red-50 text-red-700 border-red-200",             icon: <AlertTriangle className="w-3.5 h-3.5 text-red-500" /> },
  maintenance: { label: "Mant.",        dot: "bg-slate-400",                badge: "bg-slate-100 text-slate-500 border-slate-200",      icon: <RefreshCcw className="w-3.5 h-3.5 text-slate-400" /> },
  offline:     { label: "Offline",      dot: "bg-red-600",                  badge: "bg-red-50 text-red-700 border-red-200",             icon: <AlertTriangle className="w-3.5 h-3.5 text-red-600" /> },
};
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1 text-[10px] text-slate-400">{icon}{label}</div>
...
function ServerDetail({ sv }: { sv: Server }) {
    ...
    <div className="bg-slate-50 rounded-xl border border-slate-100 p-4 space-y-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">{sv.role} · {sv.os}</p>
        </div>
      ...
      {/* Mini chart */}
      <div>
        <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide mb-1.5">CPU & RAM — últimas 24 h</p>
        <ResponsiveContainer width="100%" height={90}>
          <LineChart data={sv.history} margin={{ top: 2, right: 4, left: -30, bottom: 0 }}>
            ...
            <Tooltip contentStyle={{ fontSize: 10, borderRadius: 6, border: "1px solid #e2e8f0", padding: "4px 8px" }} />
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-100">
        <div className="flex items-center gap-1 text-[10px] text-slate-400">
          ...
          Último parche: <span className="font-semibold text-slate-600 ml-0.5">{sv.lastPatch}</span>
        </div>
        <a href={`/it/servidores/${sv.id}`} className="flex items-center gap-0.5 text-[10px] font-semibold text-indigo-600 hover:text-indigo-800">
...
function ServerRow({ sv, selected, onSelect }: { sv: Server; selected: boolean; onSelect: () => void }) {
  const st  = STATUS[sv.status];
  ...
  return (
    ...
    >
      ...
      <td className={`px-3 py-2.5 text-[11px] font-bold hidden lg:table-cell ${tempColor(sv.temp)}`}>{sv.temp}°C</td>
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-emerald-50 border border-emerald-100 text-[10px] font-bold text-emerald-700">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />{counts.online} Online
          </span>
          {counts.warning > 0 && (
            <span className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-amber-50 border border-amber-100 text-[10px] font-bold text-amber-700">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />{counts.warning} Advertencia
            </span>
          )}
          {counts.critical > 0 && (
            <span className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-red-50 border border-red-100 text-[10px] font-bold text-red-700">
              <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />{counts.critical} Crítico
            </span>
          )}
          <span className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-slate-100 border border-slate-200 text-[10px] font-bold text-slate-500">
            <span className="w-1.5 h-1.5 rounded-full bg-slate-400" />{counts.maintenance} Mant.
          </span>
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
        <p className="text-[11px] text-slate-400 flex items-center gap-1">
          <Clock className="w-3 h-3" /> Datos de Azure Monitor · actualización cada 2 min
        </p>
        <a href="/it/servidores" className="flex items-center gap-1 text-[11px] font-semibold text-indigo-600 hover:text-indigo-800">
