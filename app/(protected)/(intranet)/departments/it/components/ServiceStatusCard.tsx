/**
 * @module ServiceStatusCard
 * Tarjeta de estado de servicios del módulo de Tecnología (TI).
 *
 * @remarks
 * Este componente servidor muestra el estado operativo de los principales
 * servicios corporativos monitoreados, incluyendo:
 * - Estado actual por servicio
 * - Resumen agregado por tipo de estado
 * - Banner de alerta cuando existen incidencias
 * - Métricas complementarias como uptime, tiempo activo y última verificación
 *
 * Está diseñado para renderizarse en servidor y recibir datos desde
 * {@link ITData}, con soporte para un conjunto de datos fallback.
 */
...
/**
 * Tipo de elemento individual de servicio proveniente de {@link ITData}.
 */
...
/**
 * Configuración visual y semántica por estado del servicio.
 *
 * @remarks
 * Cada estado define:
 * - Etiqueta visible
 ...
 * - Fondo y borde
 * - Punto indicador
 * - Color del borde lateral de la fila
 ...
 *
 * @remarks
 * Permite personalizar la representación visual de cada sistema monitoreado.
 * Si un servicio no tiene ícono configurado, se usa {@link Globe} como fallback.
 */
...
/**
 * Tiempo desde la última verificación por servicio.
 */
...
/**
 * Tiempo acumulado de estabilidad o actividad por servicio.
 */
 ...
 *
 * @property services Lista opcional de servicios a mostrar.
 *
 * @remarks
 * Si no se proporciona `services`, se utiliza {@link FALLBACK}.
 */
interface Props {
...
/**
 * Dataset fallback para renderizado local o cuando no hay datos disponibles.
 *
 * @remarks
 * Este conjunto permite visualizar la tarjeta incluso sin respuesta del backend.
 */
...
/**
 * Tarjeta de estado de servicios corporativos.
 *
 * @param props Propiedades del componente.
 * @returns Panel con resumen agregado y detalle de servicios monitoreados.
 *
 * @remarks
 * Flujo del componente:
 * 1. Calcula conteos por estado de servicio
 * 2. Determina si todos los servicios están operativos
 * 3. Muestra banner de alerta cuando hay degradaciones o caídas
 * 4. Renderiza una grilla con cada servicio y sus métricas
 * 5. Presenta un footer con enlace al historial completo
 *
 * Características:
 * - Soporta render con datos reales o fallback
 * - Usa guards explícitos al acceder a configuraciones derivadas
 * - Presenta indicadores compactos y escaneables
 *
 * @example
 * ```tsx
...
export default function ServiceStatusCard({ services = FALLBACK }: Props) {
  /**
   * Conteo agregado de servicios por estado.
   */
  const counts = {
  ...
  /**
   * Indica si no existen servicios en estado degradado o caído.
   */
  const allOk = counts.outage === 0 && counts.degraded === 0;

  /**
   * Indica si existe al menos una incidencia activa.
   */
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${
                allOk ? "bg-emerald-500" : "bg-amber-400 animate-pulse"
              }`}
            />
            {allOk
              ? "Todos operativos"
              : `${counts.degraded + counts.outage} con incidencia`}
          </span>
          ...
          {(["operational", "degraded", "outage", "maintenance"] as const).map(
            (s) => {
              ...
              return (
                <span
                  key={s}
                  className={`flex items-center gap-1 px-2 py-1 rounded-lg border text-[10px] font-semibold ${cfg.bg} ${cfg.border} ${cfg.color}`}
                >
      ...
      {hasAlert && (
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-3">
          ...
          <p className="text-[11px] font-medium text-amber-800">
            {counts.degraded + counts.outage} servicio
            {counts.degraded + counts.outage !== 1 ? "s" : ""} con problemas
            activos — el equipo TI está investigando
          </p>
        </div>
      )}
        ...
        {services.map((svc) => {
          ...
          return (
            ...
            >
              {/* Icon */}
              <div
                className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 border ${cfg.bg} ${cfg.border} ${cfg.color}`}
              >
                {svcIcon}
              </div>
              ...
              <div className="flex flex-col items-end gap-0.5 flex-shrink-0 text-right">
                <p className={`text-[13px] font-extrabold leading-none ${cfg.color}`}>
                  {svc.uptime}
                </p>
                <p className="text-[9px] text-slate-400">uptime</p>
                <p className="text-[9px] text-slate-400 mt-0.5">
                  activo {since} · hace {lastChk}
                </p>
              </div>
            </div>
          );
