...
const infraStats = [
  { icon: Server,      label: "Servidores activos",   value: "14 / 14",      pct: 97, color: "from-indigo-500 to-indigo-600",  textColor: "text-indigo-700" },
  { icon: Shield,      label: "Endpoints protegidos", value: "284 / 291",    pct: 98, color: "from-emerald-500 to-teal-500",   textColor: "text-emerald-700" },
  { icon: Wifi,        label: "APs en línea",          value: "21 / 21",      pct: 100,color: "from-sky-500 to-blue-500",      textColor: "text-sky-700" },
  { icon: HardDrive,   label: "Almacenamiento",        value: "68% usado",    pct: 68, color: "from-amber-500 to-orange-500",  textColor: "text-amber-700" },
];
...
function TabDisponibilidad() {
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {UPTIME_STATS.map(s => (
          <div key={s.label} className="bg-slate-50 rounded-xl px-3 py-2.5 border border-slate-100">
            ...
            <p className={`text-lg font-extrabold leading-tight mt-0.5 ${s.good ? "text-emerald-600" : "text-red-500"}`}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div>
        <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Uptime & Latencia — últimas 24 h</p>
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={uptimeData} margin={{ top: 4, right: 8, left: -22, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="uptime"   name="Uptime (%)"   stroke="#6366f1" strokeWidth={2} fill="url(#gUp)" />
            <Area type="monotone" dataKey="latencia" name="Latencia (ms)" stroke="#38bdf8" strokeWidth={2} fill="url(#gLat)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div>
        <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Errores de aplicación por hora</p>
        <ResponsiveContainer width="100%" height={120}>
          <LineChart data={uptimeData} margin={{ top: 4, right: 8, left: -22, bottom: 0 }}>
            ...
            <YAxis tick={{ fontSize: 9, fill: "#94a3b8" }} />
            <Tooltip contentStyle={{ fontSize: 11, borderRadius: 8, border: "1px solid #e2e8f0" }} />
            <Line type="monotone" dataKey="errores" name="Errores" stroke="#f43f5e" strokeWidth={2} dot={{ r: 2, fill: "#f43f5e" }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
...
function TabTickets() {
  return (
    <div className="space-y-5">
      ...
      {/* Volumen semanal */}
      <div>
        <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Volumen semanal — últimas 7 semanas</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* Pie: categorías */}
        <div>
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Por categoría</p>
          <div className="flex items-center gap-3">
            <ResponsiveContainer width={100} height={100}>
              <PieChart>
                <Pie data={ticketsByCategory} dataKey="value" cx="50%" cy="50%" innerRadius={28} outerRadius={46} strokeWidth={0}>
                  {ticketsByCategory.map((e) => <Cell key={e.name} fill={e.color} />)}
                </Pie>
              </PieChart>
            </ResponsiveContainer>
        ...
        {/* Line: SLA histórico */}
        <div>
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Cumplimiento SLA — 6 meses</p>
          <ResponsiveContainer width="100%" height={100}>
            <LineChart data={slaData} margin={{ top: 4, right: 8, left: -28, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ fontSize: 11, borderRadius: 8, border: "1px solid #e2e8f0" }} formatter={(v: number | undefined) => [v != null ? `${v}%` : "—", "SLA"]} />
              <Line type="monotone" dataKey="cumplimiento" stroke="#10b981" strokeWidth={2.5} dot={{ r: 3, fill: "#10b981" }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {infraStats.map((s) => {
          const Icon = s.icon;
          return (
            <div key={s.label} className="bg-slate-50 rounded-xl p-3.5 border border-slate-100 flex flex-col gap-2">
      ...
      {/* Server table */}
      <div>
        <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Servidores — CPU & RAM en tiempo real</p>
                ...
                <tr key={sv.id} className={i % 2 === 0 ? "bg-white" : "bg-slate-50/50"}>
                  ...
                  <td className="px-3 py-2">
                    <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold border ${
                      ...
                      <span className={`w-1 h-1 rounded-full ${sv.status === "online" ? "bg-emerald-500" : "bg-slate-400"}`} />
