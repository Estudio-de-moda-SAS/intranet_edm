...
export function FinanceSearchBar({ className = "" }: SearchBarProps) {
  ...
  const clear = useCallback(() => { setQuery(""); setOpen(false); inputRef.current?.focus(); }, []);
  ...
  return (
    <div ref={wrapperRef} className={`relative ${className}`}>
      ...
      <div className="flex items-center gap-2 rounded-xl border px-3 py-2 shadow-sm transition-all
        ...
        <Search className="h-4 w-4 shrink-0 text-slate-400 dark:text-[#545d68]" aria-hidden />
        ...
        <span className="h-4 w-px bg-slate-200 dark:bg-[#30363d] shrink-0" aria-hidden />
      ...
      {/* Dropdown */}
      {showDropdown && (
        <div className="absolute z-50 mt-1.5 w-full rounded-xl overflow-hidden
                        border shadow-xl
                        border-slate-200 bg-white
          ...
          {results.length === 0 ? (
            <div className="px-4 py-5 text-center">
              <p className="text-[12px] text-slate-500 dark:text-[#768390]">
                Sin resultados para{" "}
                <span className="font-semibold text-slate-700 dark:text-[#adbac7]">"{query}"</span>
              </p>
            </div>
          ) : (
            <>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[12.5px] font-medium truncate
                                        text-slate-800 dark:text-[#e6edf3]">{r.title}</p>
                          <p className="text-[11px] truncate
                                        text-slate-400 dark:text-[#545d68]">{r.subtitle}</p>
                        </div>
                        {r.date && (
                          <span className="text-[10px] shrink-0
                                           text-slate-400 dark:text-[#545d68]">{r.date}</span>
                        )}
                        <ArrowRight className="h-3 w-3 shrink-0 transition-colors
