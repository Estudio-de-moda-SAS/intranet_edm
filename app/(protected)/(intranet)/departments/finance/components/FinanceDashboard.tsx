  ...
  return (
    <div className="flex items-center gap-2 mb-5">
      ...
      <div>
        ...
        {sub && <p className="text-[11px] mt-0.5 text-slate-400 dark:text-[#545d68]">{sub}</p>}
      </div>
...
// Tooltip de Recharts — se pasa como prop, no puede usar Tailwind directamente.
// Se usan dos objetos: uno para light, uno para dark.
// El componente detecta dark mode en cliente para elegir cuál usar.
const TOOLTIP_LIGHT = {
  background: "#fff",
...
// Hook para detectar dark mode en cliente
function useIsDark() {
...
export default function FinanceDashboard() {
  ...
  // Colores adaptativos para ejes y grilla
  const axisColor    = isDark ? "#545d68" : "#94a3b8";
  const gridColor    = isDark ? "#21262d" : "#f1f5f9";
  // Bar de "salidas" en cash flow — en light es slate muy claro, en dark surface sutil
    ...
    <PageEnter className="space-y-10">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={MONTHLY_REVENUE}>
                ...
                <YAxis tick={{ fontSize: 11, fill: axisColor }} axisLine={false} tickLine={false} width={52}
              ...
              <div className="h-52">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={CASH_FLOW} barGap={4}>
                    ...
                    <Bar dataKey="salidas"  name="Salidas"  fill={salidasFill} stroke={salidasStroke} strokeWidth={1} radius={[4,4,0,0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="rounded-xl border p-5 shadow-sm
              ...
              <div className="flex items-center gap-4">
                <div className="h-44 w-44 shrink-0">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={EXPENSE_CATEGORIES} cx="50%" cy="50%"
                  ...
                  {EXPENSE_CATEGORIES.map((cat) => (
                    <li key={cat.name} className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[12px] text-slate-600 dark:text-[#adbac7]">{cat.name}</span>
                      </div>
      ...
      <AnimateItem>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="rounded-xl border shadow-sm
              ...
              <ul className="divide-y divide-slate-50 dark:divide-[#21262d]">
                {TRANSACTIONS.map((tx, i) => (
                  <li key={i} className="flex items-center gap-3 px-5 py-3.5 transition-colors
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[11px] text-slate-400 dark:text-[#545d68]">{tx.date}</span>
                        {tx.status === "pending" && (
          ...
          {/* Accounts receivable */}
          <div>
            <SectionHeading icon={Receipt} label="Cuentas por Cobrar"
            ...
            <div className="rounded-xl border shadow-sm
              ...
              <div className="flex gap-3 px-5 py-4 border-b border-slate-100 dark:border-[#21262d]">
                {[
                  { label: "Al día",   count: RECEIVABLES.filter(r => r.status === "current").length,  color: "bg-emerald-50 border-emerald-100 text-emerald-700 dark:bg-emerald-500/[0.10] dark:border-emerald-500/20 dark:text-emerald-400" },
                  { label: "Vencidas", count: RECEIVABLES.filter(r => r.status === "overdue").length,  color: "bg-amber-50 border-amber-100 text-amber-700 dark:bg-amber-500/[0.10] dark:border-amber-500/20 dark:text-amber-400"           },
                  { label: "Críticas", count: RECEIVABLES.filter(r => r.status === "critical").length, color: "bg-rose-50 border-rose-100 text-rose-700 dark:bg-rose-500/[0.10] dark:border-rose-500/20 dark:text-rose-400"                 },
                ].map((chip) => (
              ...
              <ul className="divide-y divide-slate-50 dark:divide-[#21262d]">
                {RECEIVABLES.map((rec, i) => {
                  const cfg =
                    rec.status === "critical" ? { dot: "bg-rose-400",   text: "text-rose-600 dark:text-rose-400",     badge: "bg-rose-50 border-rose-100 dark:bg-rose-500/[0.10] dark:border-rose-500/20",       label: "Crítico" } :
                    rec.status === "overdue"  ? { dot: "bg-amber-400",  text: "text-amber-600 dark:text-amber-400",   badge: "bg-amber-50 border-amber-100 dark:bg-amber-500/[0.10] dark:border-amber-500/20",     label: "Vencida" } :
                                                { dot: "bg-emerald-400", text: "text-emerald-600 dark:text-emerald-400",badge: "bg-emerald-50 border-emerald-100 dark:bg-emerald-500/[0.10] dark:border-emerald-500/20", label: "Al día"  };
                  return (
                    <li key={i} className="flex items-center gap-3 px-5 py-3.5 transition-colors
                                           hover:bg-slate-50/50 dark:hover:bg-[#1c2128]">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-medium truncate
                                      text-slate-700 dark:text-[#cdd9e5]">{rec.client}</p>
                        <p className="text-[11px] text-slate-400 dark:text-[#545d68]">{rec.days} días</p>
                      </div>
                      <div className="flex items-center gap-2">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-[#21262d]">
              {[
                ...
                return (
                  <tr key={i} className={`transition-colors hover:bg-slate-50/50 dark:hover:bg-[#1c2128] ${
