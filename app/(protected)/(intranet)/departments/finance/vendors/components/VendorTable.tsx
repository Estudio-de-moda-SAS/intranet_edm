...
/**
 * @module VendorTable
 * Tabla interactiva para la gestión y visualización de proveedores.
 *
 * @remarks
 * Este módulo presenta una vista administrativa del catálogo de proveedores
 * del área financiera. Incluye herramientas de:
 *
 * - búsqueda textual
 * - filtrado por estado
 * - filtrado por tipo de proveedor
 * - ordenamiento por distintos criterios
 * - paginación
 * - visualización detallada en panel lateral
 * - consulta de documentos asociados mediante visor PDF
 *
 * La tabla está diseñada para facilitar la revisión operativa,
 * financiera y documental de cada proveedor.
 */
...
/**
 * Claves válidas para ordenar la tabla de proveedores.
 *
 * @remarks
 * Cada valor representa una dimensión de ordenamiento disponible
...
/**
 * Número máximo de proveedores mostrados por página.
 */
const PAGE_SIZE = 8;
 ...
 *
 * @remarks
 * Este mapa traduce el estado funcional del proveedor
 * a una representación visual homogénea en badges y paneles.
 *
 * Incluye:
 * - etiqueta legible
 * - color de fondo
...
/**
 * Lista de tipos disponibles para filtrado.
 */
...
/**
 * Formatea un valor monetario en notación compacta.
 *
 * @param n Valor monetario a formatear.
 * @returns Cadena abreviada en formato monetario.
 *
 * @remarks
 * Este formato resulta útil para columnas de tabla
...
/**
 * Calcula el promedio general de evaluación de un proveedor.
 *
 * @param v Proveedor a evaluar.
 * @returns Promedio numérico con un decimal o `null` si no existe score.
 *
 * @remarks
 * El promedio se calcula a partir de cinco dimensiones:
...
/**
 * Determina la clase visual según el valor del score.
 *
 * @param s Puntuación numérica.
 * @returns Clase CSS para representar el nivel del puntaje.
 *
 * @remarks
 * La convención visual aplicada es:
 * - verde para puntajes altos
 * - ámbar para puntajes medios
...
/**
 * Props del componente {@link VendorTable}.
 *
 * @property vendors Lista de proveedores a mostrar en la tabla.
 */
interface Props {
  vendors: Vendor[];
}
 ...
 * @returns Etiqueta visual con el estado actual del proveedor.
 *
 * @remarks
 * Este componente toma la configuración desde {@link STATUS_CFG}
 * para mantener una semántica visual consistente.
 */
...
/**
 * Fila de estrellas para representar una puntuación resumida.
 *
 * @param props Propiedades del componente.
 * @returns Conjunto de cinco estrellas con relleno según el puntaje.
 *
 * @remarks
 * El valor se redondea al entero más cercano para decidir
 * cuántas estrellas se muestran activas.
 */
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star
          key={i}
          className={`h-2.5 w-2.5 ${i <= Math.round(score) ? 'text-amber-400 fill-amber-400' : 'text-slate-200 fill-slate-200'}`}
        />
      ))}
    </div>
  );
...
/**
 * Props del componente {@link VendorDrawer}.
 *
 * @property vendor Proveedor seleccionado.
 * @property onClose Función para cerrar el panel lateral.
 * @property onOpenPdf Función para abrir un documento PDF asociado.
 */
 ...
 *
 * @param props Propiedades del componente.
 * @returns Drawer lateral con información operativa, financiera y documental.
 *
 * @remarks
 * Este componente muestra:
 * - datos generales del proveedor
 * - estado y categoría
 * - contacto principal y contactos adicionales
 * - información bancaria
 * - historial financiero
 * - evaluación de desempeño
 * - documentos asociados
 *
 * También permite abrir documentos PDF enlazados al proveedor.
 */
function VendorDrawer({ vendor: v, onClose, onOpenPdf }: VendorDrawerProps) {
  const cfg = STATUS_CFG[v.status];
  const score = avgScore(v);

  /**
   * Indica si la sección de documentos se encuentra expandida.
   */
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <section>
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50/70 p-4 space-y-2">
              ...
              {v.city && (
                <p className="text-[12px] text-slate-400 pl-5">
                  {v.address ? `${v.address}, ` : ''}
                  {v.city}
                </p>
              )}
                ...
                <div className="px-4 py-3 bg-cyan-50 border-t border-cyan-100 flex items-center justify-between">
                  <div>
                    <p className="text-[11px] text-slate-500">
                      Revisado por <span className="font-semibold text-slate-700">{v.score.reviewedBy}</span>
                    </p>
 ...
 * @property sortKey Clave de ordenamiento representada por el botón.
 ...
 *
 * @param props Propiedades del componente.
 * @returns Botón con indicador visual del estado activo.
 */
 ...
 *
 * @remarks
 * Este componente administra el estado de interacción de la tabla:
 *
 * - texto de búsqueda
 * - estado filtrado
 * - tipo filtrado
 ...
 *
 * El procesamiento de proveedores visibles se memoiza mediante `useMemo`
 * para evitar cálculos innecesarios en cada render.
 *
 * @example
 * ```tsx
...
export function VendorTable({ vendors }: Props) {
  ...
  /**
   * Tipo de proveedor seleccionado como filtro.
   */
  ...
  /**
   * Proveedor seleccionado para mostrar en el drawer.
   */
  const [selected, setSelected] = useState<Vendor | null>(null);

  /**
   * Metadatos del documento PDF abierto.
   */
   ...
   *
   * @remarks
   * Se usa un pequeño retraso antes de limpiar `pdfMeta`
   * para permitir una transición visual más suave.
   */
  const closePdf = () => {
    setPdfOpen(false);
   ...
   *
   * @remarks
   * Si el usuario vuelve a seleccionar la clave ya activa,
   * la dirección cambia entre ascendente y descendente.
  ...
  /**
   * Lista de proveedores filtrados y ordenados.
   *
   * @remarks
   * Este cálculo:
   ...
   *
   * Se memoriza para optimizar el rendimiento del componente.
   */
  ...
  /**
   * Subconjunto de proveedores correspondiente a la página actual.
   */
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {ALL_STATUSES.map(s => {
          ...
          return (
            ...
            >
              {!isAll && (
                <span className={`h-2 w-2 rounded-full ${active ? 'bg-white/70' : STATUS_CFG[s as VendorStatus].dot}`} />
              )}
              {isAll ? 'Todos' : s}
                      ...
                      <td className="py-3.5 pl-3 pr-5">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className="h-7 w-7 flex items-center justify-center rounded-lg text-slate-400 hover:bg-cyan-100 hover:text-cyan-600 transition"
                            title="Ver ficha"
                          >
                            <Eye className="h-3.5 w-3.5" />
                          </button>
                ...
                <button
                  ...
                  className={`h-7 w-7 rounded-lg text-[12px] font-medium transition ${
                    n === page
                      ...
                      : 'border border-slate-200 text-slate-500 hover:bg-white hover:text-cyan-600'
                  }`}
                >
                  {n}
                </button>
